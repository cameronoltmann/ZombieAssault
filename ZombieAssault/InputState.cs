...
namespace ZombieAssault
{
	...
	class InputState
	{
		...
		protected KeyboardState ks_old;
		protected MouseState ms_old;
		public int X {
			get {
				return MS.X;
			}
		...
		protected bool IsDown (Buttons button, bool old=false) {
			MouseState ms = MS;
			KeyboardState ks = KS;
			if (old) {
				ms = ms_old;
			...
			if ((Buttons.MouseBegin < button) && (button < Buttons.MouseEnd)) {
				switch (button) {
				...
				case Buttons.MWUp:
					return ms.ScrollWheelValue > ms_old.ScrollWheelValue;
				case Buttons.MWDown:
					return ms.ScrollWheelValue < ms_old.ScrollWheelValue;
				default:
					return false;
			...
			else return ks.IsKeyDown ((Keys)button);
		}

		protected bool WasDown (Buttons button) {
			return IsDown (button, true);
		}

		protected bool IsUp (Buttons button) {
			return !IsDown (button);
		}

		protected bool WasUp (Buttons button) {
			return !IsDown (button, true);
		}
